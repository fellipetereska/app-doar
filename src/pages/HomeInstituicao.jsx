  ...
  useEffect(() => {
    const verificarOverflow = () => {
      if (scrollRefRecebidas.current) {
        const overflow = scrollRefRecebidas.current.scrollWidth > scrollRefRecebidas.current.clientWidth;
        setMostrarSetasRecebidas(overflow);
      }

      if (scrollRefGerais.current) {
        const overflow = scrollRefGerais.current.scrollWidth > scrollRefGerais.current.clientWidth;
  ...
  return (
    <div className="min-h-screen flex flex-col px-4 py-2">
      ...
      <div className='grid grid-cols-5 gap-4'>
        ...
        <Indicador title={`Doações Recebidas ${mes}/${String(ano).slice(-2)}`} value={indicadores.recebidasMes} borderColor="border-red-500" textColor="text-red-500"></Indicador>
        <Indicador title={`Doações Enviadas ${ano}`} value={indicadores.entregasAno} borderColor="border-yellow-500" textColor="text-yellow-500"></Indicador>
        <Indicador title={`Doações Enviadas ${mes}/${String(ano).slice(-2)}`} value={indicadores.entregasMes} borderColor="border-green-500" textColor="text-green-500"></Indicador>
      ...
      <div className="mt-4">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm border 
                  ${statusDonation === 'recebidas' ? 'bg-sky-600 text-white border-sky-600' : 'border-gray-300 text-gray-600'}`}
              >
                Recebidas
              </button>
              <button
                ...
                className={`px-3 py-1 rounded-full text-sm border 
                  ${statusDonation === 'aguardando' ? 'bg-sky-600 text-white border-sky-600' : 'border-gray-300 text-gray-600'}`}
              >
        ...
        <div className="relative">
          ...
          >
            {doacoesRecebidas.length > 0 ? (statusDonation === 'recebidas' ? doacoesRecebidas : doacoesAguardando).map(doacao => (
              <div key={doacao.doacao_id} className="min-w-[300px] max-w-[350px] h-[200px]">
          ...
          {mostrarSetasRecebidas && (
            ...
            >
              <div className="h-full flex items-center justify-end pr-2">
                <ChevronRight className="text-gray-600 hover:text-gray-800" size={24} />
              </div>
            </div>
          )}
        </div>
      ...
      <div className="mt-4">
        ...
        <div className="relative">
          ...
          <div
            ...
            className="flex gap-4 overflow-x-auto px-6 pb-2 custom-scrollbar scroll-smooth"
          >
            {doacoesGerais.length > 0 ? doacoesGerais.map((doacao) => (
              <div key={doacao.doacao_id} className="min-w-[300px] max-w-[350px] h-[200px]">
                <CardDoacao doacao={doacao} onEdit={handleEdit} />
              </div>
            )) : (
