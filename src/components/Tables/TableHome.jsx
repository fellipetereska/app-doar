...
function TableDefault({ columns, data, onEdit, onDelete, itemsPerPage = 3 }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="max-h-[75vh] overflow-auto custom-scrollbar rounded-md shadow-md">
        <table className="min-w-full bg-white text-sm text-center">
          <thead className="border border-gray-200 text-gray-700 sticky top-0">
            <tr>
              ...
              {(onEdit || onDelete) && <th className="font-medium">Ações</th>}
            </tr>
          </thead>
          ...
          <tbody className="divide-y divide-gray-100">
            {currentData.map((row, idx) => (
              <tr key={idx} className="hover:bg-gray-50">
                ...
                {(onEdit || onDelete) && (
                  <td className="px-6 py-2 flex justify-center items-center gap-2 text-base">
                    {onEdit && (
                      <button
                        onClick={() => onEdit(row)}
        ...
        <div className="flex justify-between items-center px-5 py-2 bg-white border border-gray-200 rounded-b-md shadow-md text-sm text-gray-800">
          <p className=''>
